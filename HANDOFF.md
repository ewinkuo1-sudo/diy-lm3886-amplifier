...
更新：2026-09-20，第二批零件下單（電阻／WIMA／CDE／ROE，二擇一定案 ROE）；2026-09-19，封裝庫加入絲印極性／pin 1 標記（板檔未重建）；2026-09-18，舊版 PCB 板檔與腳本已歸檔至 archive/；PCB 正反面、組裝、3D 與零件核對資料完成；PCB 為 B 方案 V0.3，原理圖／PDF 維持既有 V0.3。使用者指定**內建供電、純後級，外接現有 DAC 與前級**。已確認 Eversolo DAC-Z10（DAC＋前級）及 RCA；音量由 Z10 控制。

## 本次更新（2026-09-20）：第二批零件下單
