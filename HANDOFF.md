...
5b. **PCB V0.4 放大板**：變體 C 已於 2026-09-24 畫好（`pcb/mono-layout-v04c.*`，DRC 0，見 pcb/README「PCB V0.4」節）；接著畫變體 D（同位置、底層整面 GND 鋪銅，生成器要加 zone）與電源板 V0.4（原理圖已含 J204／snubber），三張並排預覽給使用者選。重建指令：`kicad-cli sch export netlist … electrical/netlist.xml` → KiCad 內建 python 跑 `tools/build_pcb_v04.py` → `kicad-cli pcb drc --format json --exit-code-violations -o pcb/mono-v04c-drc.json pcb/mono-layout-v04c.kicad_pcb` → `py -3.13 tools/verify_pcb_v04.py` → `tools/render_pcb_v04.py`；跑完 `git checkout -- pcb/DraftV03.pretty`（生成器只會重寫 UUID）。
