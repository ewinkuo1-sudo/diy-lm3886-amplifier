...
**兩片 100×90 mm 單聲道放大板＋一片 160×120 mm 共用電源板。**（2026-09-23 生成器已改為 **115×90 mm** 放大板，板檔未重建；下方數字仍是 100×90 版的結果。）
...
**2026-09-23 依實量改（板檔未重建）：** ROE EGW 47µF 實量軸向 40×Ø20、腳徑約 0.8–1.0 → 新增 `BP_Axial_L40_D20_P50`（腳距 50、鑽孔 1.2、焊盤 2.6、外框 40×21），C2 躺式直放於 (72,80) 轉 90°，C2.2 在上方距 R3.1 約 12 mm、C2.1 在下方以 1.0 mm B.Cu 回 SG；放大板 100→**115×90**，R8／C8／JP1 右移至新增區（R8 100,48／C8 103,66 轉 180／JP1 110,52），R2 接地改繞 (76,26)(76,43) 避開 C2.2，靜音供電 VEE 改走 F.Cu 沿 y=76 避開 C2.1 接地線。`CP_D12.5_P5` 鑽孔 0.9→1.0（EKE、361R 腳徑 0.8）。以座標自寫幾何檢查（0.3 mm 同層走線／焊盤淨距、本體重疊、板邊）無新增違規，**不是 KiCad DRC**，走線寬度與訊號完整性待重建後複查。
