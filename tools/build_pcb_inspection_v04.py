"""V0.4 review drawings, provisional VRML models and part audit (derived from the V0.3 script; boards: mono D + psu).
"""
...
R=Path(__file__).resolve().parents[1]; P=R/'pcb'; O=P/'inspection-v04'
...
def add_models(src, dest, parts):
 s=src.read_text(encoding='utf-8');out=[];i=0;n=0
 ...
 dest.write_text(''.join(out),encoding='utf-8');return n
...
manifest={'baseline':'2e31133','scope':'review artifacts only; PCB V0.4 (mono D + psu) layout unchanged','sources':{},'boards':{},'model_units':'1 VRML unit = 2.54 mm; bodies generated in mm then scaled','height_status':'All heights are illustrative assumptions, not verified or guaranteed maximum envelopes.'}
allrows=[]
for key in ['mono','psu']:
 name=NAME[key];data=json.loads((P/(name+'.json')).read_text(encoding='utf-8'));parts={c['ref']:c for c in data['parts']}
 ...
 rows=['# '+('單聲道放大板' if key=='mono' else '共用電源板')+'：組裝與接腳對照','', '2026-09-24；PCB V0.4 工程草稿（放大板變體 D、電源板 V0.4）。下列為板檔網路，不代表實物腳位已核准。','', '兩片單聲道板絲印相同；右聲道原理圖編號請看 [左右聲道對照](../../mono-channel-mapping.csv)。' if key=='mono' else 'BR1／BR2 是 4 位端子（KBPC2510 鎖機殼，Faston 線接入）；C207–C210／R203／R204 為 snubber 預留位，只留孔不填料。','', '| 板上編號 | 元件值 | 焊盤號＝網路 | 注意事項 |','|---|---|---|---|']
 for c in data['parts']:
  ...
  ORDERED={'U1':'已訂（拆機）到貨未回報；封裝未核對','C1':'已購 WIMA MKS4 100V 已到貨待量','C2':'已購 ROE EGW 已量 L40×Ø20；改躺式 P50 封裝，放大板加寬至 115 mm','C3':'已購 WIMA MKS2 100V 已到貨待量','C4':'已購 WIMA MKS2 100V 已到貨待量','C5':'已購 WIMA MKS2 100V 已到貨待量','C6':'已購 ROE EKE 已到貨待量；量直徑腳距','C7':'已購 ROE EKE 已到貨待量；量直徑腳距','C8':'已購 CDE 361R 100V 已到貨待量；量直徑','R1':'已購 MRS25 已到貨待量','R2':'已購 MRS25 已到貨待量','R3':'已購 MRS25 已到貨待量','R4':'已購 MRS25 已到貨待量','R5':'已購 RNU2 已到貨待量','R6':'已購 MRS25 已到貨待量','R7':'已購 PR02 已到貨待量','R8':'已購 MRS25 0.6W 已到貨待量','C207':'snubber 預留位，只留孔不填料（值待變壓器實測）','C208':'snubber 預留位，只留孔不填料','C209':'snubber 預留位，只留孔不填料','C210':'snubber 預留位，只留孔不填料','R203':'snubber 預留位，只留孔不填料','R204':'snubber 預留位，只留孔不填料','J204':'待購（與 J203 同款 3P 5.08 端子）','C201':'已購 CDE 381LX 已到貨待量；量直徑高度腳距','C202':'已購 CDE 381LX 已到貨待量；量直徑高度腳距','C203':'已購 CDE 381LX 已到貨待量；量直徑高度腳距','C204':'已購 CDE 381LX 已到貨待量；量直徑高度腳距','C205':'已購 WIMA MKS2 100V 已到貨待量','C206':'已購 WIMA MKS2 100V 已到貨待量','R201':'已購 PR02 已到貨待量','R202':'已購 PR02 已到貨待量'}
 ...
 for view in ['top','bottom','assembly']:
  ...
  txt(60,46,('單聲道放大板' if key=='mono' else '共用電源板')+' / PCB V0.4'+('（變體 D，底層整面接地）' if key=='mono' else ''),26,'#f1f7f6')
  txt(60,82,{'top':'正面銅箔 F.Cu｜由元件面觀看','bottom':'背面銅箔 B.Cu｜由板底觀看'+('｜暗藍面為 GND 鋪銅' if key=='mono' else ''),'assembly':'正面組裝圖｜元件位置、焊盤編號與極性'}[view],20)
  ...
  if view=='bottom':
   # V0.4 D: draw the filled B.Cu ground pour (outer ring minus holes) under the tracks.
   for zn in data.get('zones',[]):
    if zn['layer']!='B.Cu':continue
    for ring in zn['filled']:
     dd=''.join('M '+' L '.join(f'{project(*q)[0]:.2f} {project(*q)[1]:.2f}' for q in poly)+' Z ' for poly in [ring['outer']]+ring['holes'])
     svg.append(f'<path d="{dd}" fill="#3b6f9c" fill-rule="evenodd"/>')
  if view!='assembly':
  ...
  for c in data['parts']:
   ...
   txt(cx+(26 if c['ref']=='JP1' else 0),cy+(30 if c['ref']=='J5' else 4),c['ref'],size,'#ffffff','middle')
  for p in data['pads']:
