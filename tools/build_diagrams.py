...
class SVG:
    ...
    def footer(self,s):
        ...
        self.text(45,self.h-20,'方案 C PCB V0.4  •  2026-09-24  •  工程草稿／無硬體量測  •  docs/diagrams/README.md',19,GRAY)
    def save(self,name):
...
def build():
    ...
    # 2026-09-24: the partial 'complete' overview is retired to archive-v03/; build_diagram_full_v04.py draws the fully wired guide.
    d=SVG(1800,1200);d.header('LM3886｜輸入級與回授網路','元件編號與 KiCad 原理圖及 PCB V0.4 絲印一致；圖示左聲道，音量由 Eversolo DAC-Z10 控制。')
    ...
    d.panel(50,980,1700,'PCB V0.4 對應',['R4 由 U1.3 取樣；變體 D 主輸出走頂層、底層整面接地。回授位於 L1/R7 之前，元件值與網路未改。','成機需另加 DC 偵測、延遲接通與掉電斷開控制；目前不能把 TEST OUT 當完成保護的喇叭端。'],GREEN)
    ...
    d.panel(905,980,845,'局部去耦（左聲道，右聲道編號 +100）',['VCC–GND：C3 100nF / 100V；C7 470µF / 63V。','GND–VEE：C4 100nF / 100V；C6 470µF / 63V。','C7 正端接 VCC；C6 正端接 GND。','PCB V0.4：C3 跨 pin 5–7、C4 跨 pin 4–7 貼 IC 腳；','變體 D 底層整面接地，變體 C 各分路回 STAR。','GND 網名不表示各路電流應共用同一段細線。'],GREEN)
    ...
    d.panel(905,1290,845,'供電條件與選型狀態',['22×√2 − 2×1.1 ≈ 28.9V／軌：未扣紋波／下陷。','35.8V／軌是高市電空載假設，非最大電壓保證。','變壓器與全部 R／C 已到貨（9/22）；LM3886T 到貨未回報。','橋堆定案 KBPC2510 鎖機殼（未購）；保險絲、電感、接頭、浪湧控制及散熱未購。'],ORANGE)
    ...
    d.footer('四顆主電容 CDE 381LX 已到貨；橋堆定案 KBPC2510 鎖機殼但未購；未宣稱實際輸出額定。');d.save('lm3886_power_to_ic')
    ...
    sources=['tools/build_schematic.py','tools/build_power_supply.py','tools/build_diagrams.py','tools/build_diagram_full_v04.py','electrical/preview/lm3886-v01.pdf','electrical/preview/internal-psu-v02.pdf']
    def digest(p):
        b=(ROOT/p).read_bytes()
        if p.endswith('.py'): b=b.replace(b'\r\n',b'\n')  # LF-normalise text so Windows autocrlf checkouts match the repository hash
        return hashlib.sha256(b).hexdigest()
    (OUT/'sources.json').write_text(json.dumps({'version':'Plan C PCB V0.4','updated':'2026-09-24','note':'kicad_sch and pcb/README.md removed from binding: sch UUIDs change per rebuild, README changes independently of the diagrams. Text sources hashed LF-normalised.','sha256':{p:digest(p) for p in sources}},indent=2,ensure_ascii=False),encoding='utf-8')

def annotated(source,name,psu):
    d=SVG(2200,1940)
    d.header('LM3886｜'+('電源次級原理圖色塊導讀' if psu else '雙聲道放大板原理圖色塊導讀'),'底圖直接取自現行 KiCad 向量 PDF（2026-09-24 匯出）；彩色框為功能區域，不代表額外接線。')
    ...
    if psu:
        region(.012,.073,.415,.557,ORANGE);region(.50,.109,.27,.506,GREEN);region(.842,.296,.068,.18,BLUE)
        d.panel(40,1680,670,'橙框｜雙橋整流＋snubber 預留',['兩組獨立 22Vac 分別進 BR1／BR2（4 位端子）。','AC 不互接；BR1− 與 BR2+ 接 GND。'],ORANGE)
        d.panel(765,1680,670,'綠框｜濾波、洩放',['C201–C204 各 10,000µF / 63V。','負軌電容正端朝 GND；每軌 20,000µF。'],GREEN)
        d.panel(1490,1680,670,'藍框｜輸出線束',['J203 → 左聲道 J5；J204 → 右聲道 J5。','VCC／GND／VEE；橋堆、保險絲、接頭仍待購。'],BLUE)
    else:
        for dy in (0,.342):   # L channel, then R channel (same layout, offset down the page)
            region(.033,.095+dy,.28,.132,BLUE);region(.303,.219+dy,.083,.169,ORANGE);region(.422,.036+dy,.171,.264,GREEN);region(.609,.065+dy,.285,.338,PURPLE)
        ...
        d.panel(1380,1680,780,'紫框｜去耦與靜音',['負軌 C6、靜音 C8 的正端接 GND。','PCB V0.4 把 C3/C4 貼在 IC 腳上，網路未改。'],PURPLE)
    d.footer('來源：electrical/preview/'+source+'.pdf（2026-09-24 匯出）。');d.save(name)
    doc.close()

if __name__=='__main__':
    build()
    print('Built five SVG guides and source hashes (full wired guide: build_diagram_full_v04.py).')
