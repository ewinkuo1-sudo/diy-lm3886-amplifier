...
def analyze():
 ...
 lines=['# PCB V0.3 銅箔計算表（程式輸出）','','由 `tools/analyze_pcb_v03.py` 依 PCB 走線資料計算。完整假設與限制見 [PCB V0.3 說明](README.md)。','', '以下採名義 1 oz 銅厚 0.0348 mm、假設銅溫 25°C。電流情境為 40W／8Ω；充電列另採 15% 導通占空比矩形脈衝假設。','', '| 路徑 | RMS A | V0.2 電阻 mΩ | V0.3 電阻 mΩ | V0.3 峰值壓降 mV | V0.3 發熱 W |','|---|---:|---:|---:|---:|---:|']
 for row in rows:
