...
for name in ['docs/archive/09-採購後修訂計畫.md','docs/01-設計規格.md','docs/05-內建電源與機構.md','docs/02-功率與散熱估算.md','docs/06-市電與電源估算.md','docs/04-上電與驗收.md','docs/archive/08-採購狀態.md','electrical/validation.md']:
...
source=(ROOT/'docs/archive/08-採購狀態.md').read_text(encoding='utf-8');a=source.index('### 放大板被動元件');b=source.index('### 接插件');shopping.extend(markdown('# 放大板（全部未購）\n'+source[a:b]));shopping.append(PageBreak());shopping.extend(markdown('# 接插件與測試準備\n'+source[b:]))
