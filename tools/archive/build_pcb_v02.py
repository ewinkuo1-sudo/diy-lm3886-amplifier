...
from pcb_layout_v02 import AMP, PSU
import pcbnew as p
import xml.etree.ElementTree as ET
import math,json

ROOT=Path(__file__).resolve().parents[2]
OUT=ROOT/'pcb/archive/v02';OUT.mkdir(parents=True,exist_ok=True)
LIB=OUT/'DraftV02.pretty';LIB.mkdir(exist_ok=True)
MM=p.FromMM
def v(x,y):return p.VECTOR2I(MM(x),MM(y))
def xy(a):return (p.ToMM(a.x),p.ToMM(a.y))
specs={}
def make_fp(name,pads,body,kind='rect'):
 f=p.FOOTPRINT(None);f.SetFPID(p.LIB_ID('DraftV02',name));f.SetAttributes(p.FP_THROUGH_HOLE)
...
if __name__=='__main__':
 ...
 print('V0.2 layout B: routed engineering draft; no manufacturing release.')
