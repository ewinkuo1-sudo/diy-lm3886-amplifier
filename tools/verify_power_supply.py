...
def verify():
    ...
    expected = {
        'VCC': {'BR1.P', 'C201.1', 'C202.1', 'R201.1', 'C205.1', 'J203.1', 'J204.1'},
        'GND': {'BR1.N', 'BR2.P', 'C201.2', 'C202.2', 'R201.2', 'C205.2', 'C203.1', 'C204.1', 'R202.1', 'C206.1', 'J203.2', 'J204.2'},
        'VEE': {'BR2.N', 'C203.2', 'C204.2', 'R202.2', 'C206.2', 'J203.3', 'J204.3'},
    }
    # V0.4: per-winding snubber provision Cx || (Rs + Cs) across the secondary terminals; one output terminal per channel.
    for ch, j, f, br, cx, rs, cs in [('POS', 'J201', 'F201', 'BR1', 'C207', 'R203', 'C208'), ('NEG', 'J202', 'F202', 'BR2', 'C209', 'R204', 'C210')]:
        expected[ch+'_AC1'] = {j+'.1', f+'.1', cx+'.2', rs+'.2'}
        expected[ch+'_AC_FUSED'] = {f+'.2', br+'.AC1'}
        expected[ch+'_AC2'] = {j+'.2', br+'.AC2', cx+'.1', cs+'.1'}
        expected[ch+'_SNUB'] = {rs+'.1', cs+'.2'}
    if nets != expected:
    ...
    if len(parts) != 22:
        raise RuntimeError('Expected 22 secondary PSU components (15 + J204 + 6 snubber provisions)')
    ...
    for pin, net in [(1, 'VCC'), (2, 'GND'), (3, 'VEE')]:
        if f'J5.{pin}' not in amp_nets[net] or f'J203.{pin}' not in nets[net] or f'J204.{pin}' not in nets[net]:
    ...
    with (DEST/'validation.md').open('a') as f:
        f.write(f'\n## 內建電源次級圖\n\nERC 0 錯誤、0 警告；{len(parts)} 個元件、{len(nets)} 個網路、{sum(map(len,nets.values()))} 個腳位通過獨立核對。兩個次級 AC 回路、橋式整流輸出串接、濾波／洩放極性及 J203／J204（每聲道一組）→放大板 J5 的三線接序已核對；每組次級繞組留有 Cx‖(Rs+Cs) 緩衝提供位（V0.4，元件值待量）。\n\n這是兩個獨立 KiCad 專案，以線束相連；ERC 不會自動檢查跨檔接線。Bridge 的 AC1／AC2／P／N 為功能端子，未選實體料號或 footprint。沒有完成一次側接線、軟啟動、保護控制器、PCB、耐壓／接地測試或實機量測。\n')
