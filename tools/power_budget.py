...
TA, RJC, RCS, RSA = 40.0, 1.0, 0.3, 0.4
...
def report():
    ...
    lines += ['', '## 供電與耗散', '',
              ...
              '|---|---:|---:|---:|---:|---:|---:|']
    for rail in (28, 30, 36):
        heat = worst_heat(rail)
        dc = 2 * (30 + dissipation(rail, 30))
    ...
    lines += ['', '上表的功率上界是電壓幾何估算，不包含限流、SPiKe、失真或電源下陷，不能當額定輸出。', '',
              ...
              '2×40W 同相正弦的每軌訊號峰值約 6.32A，另有靜態電流。已訂 2×22Vac 變壓器的 VA 與繞組電流未知，不宣稱連續 DC 能力；詳見[內建電源估算](06-mains-calculations.md)。', '',
              '## 靜音電流', '',
              '依原廠近似關係：`I8 = (|VEE| − 2.6V) / 22kΩ`。電阻功耗為 `(VEE_abs−2.6)²/22k`，未含瞬態。', '',
              '| 負電源 | 解除靜音電流估算 | 22k 穩態耗散 |', '|---|---:|---:|']
    for rail in (20, 28, 30, 36):
