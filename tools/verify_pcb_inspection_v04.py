"""V0.4 inspection verification (derived from the V0.3 script; run with KiCad Python).
"""
...
import pcbnew as p
R=Path(__file__).resolve().parents[1];O=R/'pcb/inspection-v04';sources=json.loads((O/'sources.json').read_text(encoding='utf-8'));audit=json.loads((O/'parts-audit.json').read_text(encoding='utf-8'))
...
for key in ['mono','psu']:
 src=R/'pcb'/(NAME[key]+'.kicad_pcb');dst=O/'3d'/(NAME[key]+'-preview.kicad_pcb')
 assert parse(src.read_text(encoding='utf-8'))==strip_models(parse(dst.read_text(encoding='utf-8'))),f'{key}: unexpected preview mutation'
 assert (src.with_suffix('.kicad_pro')).read_bytes()==dst.with_suffix('.kicad_pro').read_bytes()
 b=p.LoadBoard(str(dst));data=json.loads((R/'pcb'/(NAME[key]+'.json')).read_text(encoding='utf-8'));fp={f.GetReference():f for f in b.GetFootprints()}
 ...
 for c in data['parts']:
  ...
  coords=[float(x)*2.54 for x in re.search(r'point \[([^]]+)\]',path.read_text(encoding='utf-8')).group(1).replace(',',' ').split()]
 ...
 for view in ['top','bottom','assembly']:
  svg=(O/'drawings'/f'{key}-{view}.svg').read_text(encoding='utf-8');assert '</svg>' in svg
 ...
 for side in ['top','bottom']:assert '</svg>' in (O/'native'/f'{key}-{side}.svg').read_text(encoding='utf-8')
log=json.loads((O/'native-export-log.json').read_text(encoding='utf-8'));assert len(log)==10 and all(x['returncode']==0 for x in log)
assert len(audit)==44
report={'date':'2026-09-24','checks':checks,'audit_rows':len(audit),'native_exports':len(log),'scope':'source/geometry/model/asset consistency only; no new ERC/DRC/hardware testing','manual_visual_review':'Top/bottom copper, assembly polarity, native top/bottom/isometric component renders reviewed; see README limitations.','files':{f.relative_to(R).as_posix():sha(f) for f in sorted(O.rglob('*')) if f.is_file() and f.name!='verification.json' and f.suffix not in ['.kicad_prl'] and 'models' not in f.parts and '-preview.' not in f.name and not f.name.startswith('raw-')}}
