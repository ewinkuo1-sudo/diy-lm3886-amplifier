"""V0.4 framed 3D renders (derived from the V0.3 script).
"""
"""Frame native 3D renders with model assumptions, using Pillow."""
from pathlib import Path
from PIL import Image,ImageDraw,ImageFont
import os
R=Path(__file__).resolve().parents[1];O=R/'pcb/inspection-v04/3d'
...
for key,label in [('mono','單聲道放大板｜每台兩片'),('psu','共用電源板｜每台一片')]:
 for v,title in [('top','正面'),('bottom','背面'),('isometric','斜視')]:
  ...
  d.text((50,78),'PCB V0.4｜KiCad 原生算繪＋暫定元件外框模型'+('｜放大板變體 D' if key=='mono' else ''),font=f(24),fill='#c0d7dc')
...
d.text((55,28),'LM3886｜PCB V0.4 立體配置預覽（放大板 D＋電源板）',font=f(42),fill='#f1f7f6')
d.text((55,90),'兩片相同放大板＋一片共用電源板；下圖各展示一種板。',font=f(27),fill='#b0c7d0')
for key,x,label in [('mono',35,'放大板（變體 D）115 × 90 mm，需兩片'),('psu',1070,'電源板 160 × 120 mm，需一片')]:
